...
Feature: Create node specialization with a custom destination

  As a user of the CR I want to create a copy of a node within an aggregate to a more specialized dimension space point.
  In these cases, another succeeding sibling and / or parent is assigned to the variant as well.
  ...
  Scenario: Create a specialization of a node to a dimension space point with further specializations and to another succeeding sibling
    ...
    And event at index 10 is of type "NodeSpecializationVariantWasCreated" with payload:
      ...
      | specializationSiblings | [{"dimensionSpacePoint":{"example":"spec"},"nodeAggregateId":"elder-mc-nodeface"},{"dimensionSpacePoint":{"example":"leafSpec"},"nodeAggregateId":"elder-mc-nodeface"}] |
  ...
  # @todo vary to a new parent which covers the requested DSPs (fully / partially), while the original parent does not
